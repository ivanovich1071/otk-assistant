...
- **карта обмера** — по одному чертежу, система из трёх агентов:
  чертёж → размеченный JPG + карта обмера в Word;
...
- `.claude/rules/sheet-zones.md` — три зоны листа: чертёж, штамп, текстовая часть;
  что из них не нумеруется никогда
